  ...
  <div class="mt-1 mx-1">
    <b-row>
      <b-col cols="12" class="mt-2 mb-2">
        <b-form inline>
          <b-form-checkbox size="sm" v-model="query.nearby">
            显示附近设备
          </b-form-checkbox>
          <b-form-input
          ...
          <b-form-checkbox class="ml-2" size="sm" v-model="query.fence">
            显示围栏
          </b-form-checkbox>

          <p class="ml-2 mt-2" v-if="currentMousePostion != null">
            当前位置: {{ currentMousePostion.latlng.lng }} ,{{
              currentMousePostion.latlng.lat
            }}
          </p>

          <p class="ml-2 mt-2" v-if="clickPostion != null">
            点击位置: {{ clickPostion.latlng.lng }} ,{{
              clickPostion.latlng.lat
            }}
          </p>
        </b-form>
      </b-col>
  ...
  methods: {
    init() {
      ...
      var map = (this.map = L.map("map", {
        ...
        [13.361389338970184, 38.11554879133282],
        _this.form.meta?.zoom ?? 18
      ));
      ...
      device
        ...
        .then((res) => {
          ...
          if (_this.query.nearby) {
            _this.circle = L.circle(
              [res.data.location.y, res.data.location.x],
              {
                color: "#00000000",
                fillColor: "#0000ff",
                fillOpacity: 0.1,
